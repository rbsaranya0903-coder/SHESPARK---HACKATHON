...
router.post('/:userId/step', async (req, res) => {
    try {
        ...
        if (data.company_name) { sql += ', company_name = ?'; params.push(data.company_name); }
        if (data.industry) { sql += ', industry = ?'; params.push(data.industry); }
        if (data.target_audience) { sql += ', target_audience = ?'; params.push(data.target_audience); }
        if (data.social_links) { sql += ', social_links = ?'; params.push(JSON.stringify(data.social_links)); }
        if (data.competitors) { sql += ', competitors = ?'; params.push(JSON.stringify(data.competitors)); }
